...
{
   if (NULL == (input_file = fopen("input.txt", "r")) 
   	|| NULL == (input_check = fopen("inputCheck.txt", "r"))) {
      return -1;
   }
   else {
      return 0;
...
void test_Input(void)
{
	...
	int MAXLINE = 256; //For Now, accept this many chars in a line. THIS SHOULD BE HANDLED IN A SEPARATE PROGRAM LOOP, but is necessary for now to test Input!
	int logicBoolean = 1; //whether or not to move on to IO test
...
int main()
{
   ...
   if (NULL == CU_add_test(pSuite, "test of Input()", test_Input))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
